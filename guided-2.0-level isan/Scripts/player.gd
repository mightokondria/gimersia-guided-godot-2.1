...
# --- VARIABEL TOGGLE KEMAMPUAN (DEBUG) ---
var can_dash = false # Ubah ke false jika ingin mulai tanpa dash
...
# [BARU] Pastikan status visual orb benar saat game mulai
func _ready():
...
func _input(event):
	if event is InputEventKey and event.pressed:
		# Tombol Angka 1: Toggle Dash
		if event.keycode == KEY_1:
			can_dash = !can_dash
			# [BARU] Ubah visibilitas orb sesuai status can_dash
			dash_orb.visible = can_dash
			print("Ability Dash: ", can_dash)
			
		# Tombol Angka 2: Toggle Double Jump
		elif event.keycode == KEY_2:
			can_double_jump = !can_double_jump
			print("Ability Double Jump: ", can_double_jump)
			
		# Tombol Angka 3: Toggle Wall Grip/Jump
		elif event.keycode == KEY_3:
			can_wall_slide = !can_wall_slide
	...
	if not is_dashing:
		if controls_enabled:
			...
			if not on_floor:
				...
				if on_wall and velocity.y > 0 and pushing_wall and not is_wall_jumping and can_wall_slide:
